...
export default async function bangaloreCityPage() {
    ...
    const bangalorePosts = allPosts.filter(post =>
        (post.title as string)?.toLowerCase().includes('bangalore') ||
        (post.slug as string)?.toLowerCase().includes('bangalore') ||
        (post.tags as string[])?.some((tag: string) => tag.toLowerCase() === 'bangalore')
    ).slice(0, 3);

    const popularAreas = [
        { name: "Koramangala", description: "The startup and social heart of Bengaluru with legendary cafes and a young, vibrant crowd." },
        { name: "Indiranagar", description: "A perfect blend of upscale residencies and the city's most happening social spots." },
        { name: "HSR Layout", description: "The new startup hub of the city, home to thousands of young professionals looking for community." },
        { name: "Whitefield", description: "The massive IT corridor where professional life meets a growing social and cultural scene." }
    ];
                    ...
                    <div className="grid lg:grid-cols-2 gap-16 items-center">
                        ...
                        <div className="relative aspect-[9/16] max-h-[700px] w-full ml-auto">
                            <Image
                                ...
                                alt="Stranger Mingle Bangalore Partner Cafes"
                                fill
